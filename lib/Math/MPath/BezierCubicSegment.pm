        ...
        foreach my $spanB (@{$XtoTLUT_B}) {
            ...
            # the zero or two intersection case
                ...
                if ($at_start > 0 && $at_end < 0 || $at_start < 0 && $at_end > 0) {
                    ...
                    #warn "PAIR!! ADAPT THIS CODE FOR OFFSET CURVES!!\n";
...
# next two are used with attempt at bezoff_bezoff_intersect()
sub t_from_xoff {
...
sub t_prime_from_xoff {
    my ($self, $xoff, $offset, $tbounds, $tprimefunc, $reversed, $e) = @_;
    
    my $xoffadjusted = $xoff - $e;
    ...
    my $find_matching_offset_x = sub {
        ...
        return $xoffadjusted - ($x0 + $offset * cos($a0));
    };

    my ($t1, $msg1) = FalsePosition($find_matching_offset_x,$tbounds,0.000001,($tbounds->[0]+$tbounds->[-1])/2,'t_prime_from_xoff -');    
    if ($msg1) {warn "false position message: $msg1";}    
    $xoffadjusted = $xoff + $e;
    my ($t2, $msg2) = FalsePosition($find_matching_offset_x,$tbounds,0.000001,($tbounds->[0]+$tbounds->[-1])/2,'t_prime_from_xoff +');    
    if ($msg2) {warn "false position message: $msg2";}    
