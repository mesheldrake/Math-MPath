...
my @intersections = Math::MPath::Intersections::intersect_CC($mp_bez_seg,$mp_bez_seg);
...
my @intersection_pair = Math::MPath::Intersections::intersect_CC($mp_bez_seg_1,$mp_bez_seg_2);

ok(    abs($intersection_pair[0]->[0] - 10.3517) < 0.0001
    && abs($intersection_pair[0]->[1] -  9.9952) < 0.0001
);
ok(    abs($intersection_pair[1]->[0] - 25.6583) < 0.0001
    && abs($intersection_pair[1]->[1] - 22.8527) < 0.0001
);
